      ...
      const row = new Discord.MessageActionRow()
        .addComponents(
          new Discord.MessageButton()
            ...
            .setEmoji('<:Caixa:1121658402448343070>')
            .setLabel('Quantidade')
            .setStyle('SECONDARY'),
        )
        .addComponents(
        ...
        const msg = await message.reply({ embeds: [new Discord.MessageEmbed()
          ...
          .setDescription(`
          <:Caixa:1121658402448343070> | Quantidade: ${db.get(`${adb}.quantidade`)}
<a:carregando_2:1121658406734934121> | Mínimo: ${db.get(`${adb}.minimo`)} 
📊 | Porcentagem: ${db.get(`${adb}.desconto`)}%`)
        ...
        interação.on("collect", async (interaction) => {
         ...
         if (interaction.customId === "mincupom") {
             interaction.deferUpdate();
             msg.channel.send("❓ | Qual o novo mínimo para uso em ?").then(msg => {
               const filter = m => m.author.id === interaction.user.id;
               const collector = msg.channel.createMessageCollector({ filter, max: 1 });
               collector.on("collect", message => {
                 message.delete()
